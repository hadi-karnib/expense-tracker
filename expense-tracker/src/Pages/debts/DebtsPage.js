...
export default function DebtsPage() {
  const { debts, loading, error, addDebt, updateDebt, deleteDebt, payDebt } =
    useDebtsData();
  ...
  const totals = useMemo(() => {
    ...
    const remaining = debts.reduce(
      (acc, d) => acc + Number(d.remainingAmount || 0),
      0,
    );
    ...
    <Stack spacing={2}>
        ...
        <CardContent>
          ...
          >
            <Chip
              ...
              label={
                <span>
                  Total: <Money amountUsd={totals.total} />
                </span>
              }
            />
            <Chip
              ...
              label={
                <span>
                  Remaining: <Money amountUsd={totals.remaining} />
                </span>
              }
            />
      ...
      {loading ? (
        ...
        <Grid container spacing={2}>
          {debts.map((d) => {
            const paid =
              Number(d.totalAmount || 0) - Number(d.remainingAmount || 0);
            const pct = d.totalAmount
              ? (paid / Number(d.totalAmount)) * 100
              : 0;
                ...
                <Card sx={{ height: "100%" }}>
                  <CardContent>
                    <Stack spacing={1.25}>
                      ...
                      >
                        <Typography variant="caption" color="text.secondary">
                          Paid: <Money amountUsd={paid} /> /{" "}
                          <Money amountUsd={d.totalAmount} />
                        </Typography>
                        {isPaidOff ? (
                          <Chip size="small" color="success" label="Paid off" />
                        ) : (
